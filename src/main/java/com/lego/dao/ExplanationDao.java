...
public class ExplanationDao {
    /**
     * 通过前面的配置可以看出，Spring 对 JPA 的支持已经非常强大，
     * 开发者无需过多关注 EntityManager 的创建、事务处理等JPA 相关的处理
     * ***********************************************************************
     ...
     * 每写一个实体类，都要衍生出5、6个类来对他进行操作，即使有了注解，
     * 我们可以依赖注入方式来拿到实现类，
     * 但是通用的CRUD等操作却不免在每个实现类里声明，
     * 你又说，我可以定义一个父类，利用泛型反射原理就可以了，
     * 但那样你还需要为每个Dao声明自己的实现类来继承你的父类
     * ***********************************************************************
     * 那么问题来了...对持久层的简化技术哪家强？
     * Spring Data Jpa
     * 你唯一要做的就只是声明持久层的接口，来继承他自身已经封装好的持久层接口，
     * 正如本类UserDao一样
     * 可使用的接口有：
